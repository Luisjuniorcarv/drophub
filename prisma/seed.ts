...
async function main() {
  ...
  const productsData = [
    ...
    },
    {
      ...
      supplierId: supplier3.id,
      costPrice: 28.0,
      sellingPrice: 79.9, // Lucro: 51.90 | Margem: 64.9% | Markup: 185.3%
      stock: 45,
      status: ProductStatus.ACTIVE,
      images: [
    ...
    },
    {
      ...
      supplierId: supplier3.id,
      costPrice: 18.0,
      sellingPrice: 54.9, // Lucro: 36.90 | Margem: 67.2% | Markup: 205.0%
      stock: 60,
      status: ProductStatus.ACTIVE,
      images: [
  ...
  for (const p of productsData) {
    const { images, variants, ...prodFields } = p as any;
  ...
  console.log(`✓ ${createdProducts.length} Produtos cadastrados com imagens e variações.`);
